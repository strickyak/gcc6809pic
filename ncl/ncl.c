...
// We can use -ffixed-y on the command line, instead.
// Reserve the Y register for pointer to the process's data region.
// register int os9_data_area asm("y");
...
#define  Os9ChgDir  GccOs9ChgDir
#define  Os9Mem     GccOs9Mem
#define  Os9Exit     GccOs9Exit
#define  Os9ID     GccOs9ID
#define  Os9ReadLn     GccOs9ReadLn

#include "frob3/froblib.h"
#include "ncl/ncl.h"
#include "ncl/ncl_os9.h"
#include "ncl/match.h"
#include "ncl/regexp.h"
#include "os9/_generated_os9api_for_gcc.h"

#include "ncl/buf.c"
#include "ncl/format.c"
#include "ncl/stdlib.c"
#include "ncl/nylib.c"
#include "ncl/nystdio.c"

#include "os9/_generated_os9api_for_gcc.c"

#include "ncl/match.c"
#include "ncl/ncl_os9.c"
#include "ncl/regexp.c"
#include "ncl/malloc.c"
...
void TearDown() {
  QUICK_FIX_STATIC struct picolCmd * c;
  for (c = Commands; c;) {
    free(c->name);
    if (c->privdata) {
  ...
  free(Callframe);
  QUICK_FIX_STATIC struct picolArray * a;
  for (a = Arrays; a;) {
    free(a->name);
    TearDownVars(a->vars);
    void *tmp = a;
...
int picolCommandJoin(int argc, char **argv, void *pd) {
  /*
  ncl.c: In function 'picolCommandJoin':
ncl.c:846:1: error: unable to find a register to spill in class 'A_REGS'
ncl.c:846:1: error: this is the insn:
(insn 18 17 19 2 (set (reg:HI 1 x)
        (mem/f:HI (plus:HI (reg/v/f:HI 47 [ argv ])
                (const_int 2 [0x2])) [2 MEM[(char * *)argv_3(D) + 2B]+0 S2 A8])) ncl.c:820 10 {*movhi_1}
     (nil))
ncl.c:846: confused by earlier errors, bailing out
  */
  QUICK_FIX_STATIC int c;
  QUICK_FIX_STATIC const char **v;
  c = 0;
  v = NULL;
  {
    int err = SplitList(argv[1], &c, &v);
    if (err) {
        return picolCommandError(argc, argv, pd);
    }
  }
...
int picolCommandCallProc(int argc, char **argv, void *pd) {
  ...
  QUICK_FIX_STATIC int c;
  QUICK_FIX_STATIC const char **v;
  QUICK_FIX_STATIC int err;
  c = 0;
  v = NULL;
  err = SplitList(alist, &c, &v);
  if (err) {
    return picolCommandError(argc, argv, pd);
  }

  QUICK_FIX_STATIC byte varargs = false;
