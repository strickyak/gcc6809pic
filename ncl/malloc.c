...
void *Malloc(word n) {
  ...
  //printf("m:first:%d\n", __LINE__);
      ...
      // byte MemoryPoolForUnix[102400];
       ...
       errnum err = GccOs9Mem(new_memory_size, &new_memory_size, &end_of_new_mem);
       if (err) MallocOOM(err, n, 0);
       //printf(" *** 1st GccOs9Mem e=%x => %x %x\n", err, new_memory_size, end_of_new_mem);
       ...
       err = GccOs9Mem(new_memory_size, &new_memory_size, &end_of_new_mem);
       //printf(" *** 2nd GccOs9Mem e=%x => %x %x\n", err, new_memory_size, end_of_new_mem);
       if (err) MallocOOM(err, n, 0);
...
__attribute__((noinline))
void PrependToFreeListB(struct MallocHead *h, byte b) {
...
#if 1 // avoid GCC bug
  PrependToFreeListB(h, b);
#else

/*
In file included from ncl.c:41:0:
../ncl/malloc.c: In function 'Free':
../ncl/malloc.c:243:1: error: unable to find a register to spill in class 'A_REGS'
../ncl/malloc.c:243:1: error: this is the insn:
(insn 69 64 72 8 (set (mem/s/f:HI (reg/f:HI 57) [2 TheHeap.h_buck_freelist S2 A8])
        (reg/v/f:HI 29 [ h ])) ../ncl/malloc.c:237 10 {*movhi_1}
     (expr_list:REG_DEAD (reg/f:HI 57)
        (expr_list:REG_DEAD (reg/v/f:HI 29 [ h ])
            (nil))))
../ncl/malloc.c:243: confused by earlier errors, bailing out
make: *** [Makefile:12: ncl.os9] Error 1
